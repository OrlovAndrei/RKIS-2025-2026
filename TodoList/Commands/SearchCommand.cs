...
public class SearchCommand : ICommand
{
    ...
    public void Execute()
    {
        ...
        var items = Enumerable.Range(0, todos.Count)
                              .Select(i => (Index: i, Item: todos[i]));
        ...
        if (_sortBy == "text")
            query = _descending ? query.OrderByDescending(x => x.Item.Text)
                                 : query.OrderBy(x => x.Item.Text);
        else if (_sortBy == "date")
            query = _descending ? query.OrderByDescending(x => x.Item.LastUpdate)
