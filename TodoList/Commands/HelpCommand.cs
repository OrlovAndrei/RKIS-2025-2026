using System;

namespace Todolist
{
    public class HelpCommand : ICommand
    {
        public void Execute()
        {
            Console.WriteLine("""
            Доступные команды:
            help    - выводит список всех доступных команд с кратким описанием
            profile - выводит данные пользователя (флаги -o --out - выход из профиля)
            add     - добавляет новую задачу (флаги -m --multiline - многострочный режим)
            read    - полный просмотр задачи
            view    - выводит все задачи из массива
            search  - поиск задач по различным критериям
            status  - изменяет статус задачи
            delete  - удаляет задачу по индексу
            update  - обновляет текст задачи
            undo    - отменяет последнее действие
            redo    - повторяет отмененное действие
            load    - имитация параллельной загрузки файлов с прогресс-баром
            sync    - синхронизация данных с сервером
            exit    - завершает программу

            Команда 'load':
            Использование: load <количество_скачиваний> <размер_скачиваний>
            Пример: load 3 100
            - создает указанное количество параллельных загрузок
            - каждая загрузка показывает свой прогресс-бар
            - размер определяет количество шагов загрузки

            Команда 'sync':
            Использование: sync --pull | --push
            --pull   - получить данные с сервера и обновить локальные
            --push   - отправить локальные данные на сервер
            Пример: sync --push
            - отправляет все профили и задачи текущего пользователя на сервер
            - для работы требуется запущенный сервер (TodoList.Server)

            Флаги для команды 'view':
            -i, --index       - показывать индекс задачи
            -s, --status      - показывать статус задачи
            -d, --update-date - показывать дату изменения
            -a, --all         - показывать все данные

            Флаги для команды 'search':
            --contains <text>    - задачи, содержащие указанный текст
            --starts-with <text> - задачи, начинающиеся с указанного текста
            --ends-with <text>   - задачи, заканчивающиеся указанным текстом
            --from <date>        - задачи с датой изменения не раньше указанной (формат: yyyy-MM-dd)
            --to <date>          - задачи с датой изменения не позже указанной (формат: yyyy-MM-dd)
            --status <status>    - задачи с указанным статусом
            --sort text/date     - сортировка по тексту или дате изменения
            --desc               - сортировка по убыванию (по умолчанию - по возрастанию)
            --top <n>            - вывести только первые n задач

            Статусы задач для команд 'status' и 'search':
            NotStarted, InProgress, Completed, Postponed, Failed
            """);
        }

        public void Unexecute()
        {
            throw new NotImplementedException("Команда help не поддерживает отмену");
        }
    }
}