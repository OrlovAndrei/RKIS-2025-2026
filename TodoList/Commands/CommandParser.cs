...
namespace TodoList
{
    ...
    public static class CommandParser
    {
        ...
        private static void InitializeCommandHandlers()
        {
            ...
            _commandHandlers["sync"] = ParseSync;   // добавлено
        }
        ...
        private static ICommand ParseSync(string args)
        {
            ...
            if (!string.IsNullOrEmpty(args))
            {
                ...
                foreach (var part in parts)
                {
                    if (part == "--pull") pull = true;
                    else if (part == "--push") push = true;
                    else throw new InvalidArgumentException($"Неизвестный флаг для sync: {part}");
                }
            }

            if (pull && push) throw new InvalidArgumentException("Нельзя использовать одновременно --pull и --push");
