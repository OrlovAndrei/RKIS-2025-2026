...
namespace TodoApp.Commands
{
	public class CommandParser
	{
		...
		public BaseCommand Parse(string input, Guid? currentProfileId, TodoList todoList)
		{
		...
		private void InitializeCommandHandlers()
		{
			...
			_commandHandlers["profile"] = (input, todoList, currentProfileId) =>
				ParseProfileCommand(input, currentProfileId, _storage); 
		...
		private BaseCommand ParseReadCommand(string input, TodoList todoList, Guid? currentProfileId)
		{
			string[] parts = input.Split(' ', StringSplitOptions.RemoveEmptyEntries);
		...
		private BaseCommand ParseStatusCommand(string input, TodoList todoList, Guid? currentProfileId)
		{
			string[] parts = input.Split(' ', StringSplitOptions.RemoveEmptyEntries);
		...
		private BaseCommand ParseProfileCommand(string input, Guid? currentProfileId, IDataStorage storage)
		{
			string[] parts = input.Split(' ', StringSplitOptions.RemoveEmptyEntries);
			var command = new ProfileCommand(storage);
			command.SaveToFile = parts.Length > 1 && parts[1] == "--out";
			return command;
		}

		private BaseCommand ParseUpdateCommand(string input, TodoList todoList, Guid? currentProfileId)
		{
			string[] parts = input.Split(' ', StringSplitOptions.RemoveEmptyEntries);
		...
		private BaseCommand ParseAddCommand(string input, TodoList todoList, Guid? currentProfileId)
		{
		...
		private BaseCommand ParseViewCommand(string input, TodoList todoList, Guid? currentProfileId)
		{
		...
		private TodoStatus? ParseStatus(string statusString)
		{
		...
		private BaseCommand ParseSearchCommand(string input, TodoList todoList, Guid? currentProfileId)
		{
			try
			{
		...
		private BaseCommand ParseLoadCommand(string input)
		{
			string[] parts = input.Split(' ', StringSplitOptions.RemoveEmptyEntries);
		...
		private string ExtractTextArgument(string[] parts, ref int index)
		{
