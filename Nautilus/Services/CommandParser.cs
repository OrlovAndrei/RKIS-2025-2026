        ...
        private static ICommand ParseSearchCommand(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                else if (arg == "--sort")
                {
                    if (i + 1 < args.Length)
                    {
                        string sortArg = args[++i].ToLower();
                        if (sortArg == "date")
                            sortBy = SortАпельсинок.LastUpdat;
                        else if (sortArg == "text")
                            sortBy = SortАпельсинок.Text;
                    }
                }
                else if (arg == "--desc")
                {
                ...
                else if (arg == "--status")
                {
                    if (i + 1 < args.Length)
                    {
                        string statusArg = args[++i].ToLower().Replace("-", "");
                        if (Enum.TryParse<TodoStatus>(statusArg, ignoreCase: true, out var parsedStatus))
                        {
                            status = parsedStatus;
                        }
                    }
                ...
                {
                    if (i + 1 < args.Length && DateTime.TryParse(args[++i], out var date))
                    {
                        dateFrom = date;
                    }
                }
            ...
            return new SearchCommand(searchType, text, status, dateTo, dateFrom, sortBy, sortDesc, top);
        }
