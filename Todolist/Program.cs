...
class Program
{
    ...
    public static void SelectProfile()
    {
        while (true)
        {
            ...
            try
            {
                var loadedTodos = AppInfo.Storage.LoadTodos(AppInfo.CurrentProfile.Id).ToList();
                AppInfo.Todos = new TodoList(loadedTodos);
            }
            catch (Exception ex)
            {
    ...
    {
        if (AppInfo.CurrentProfileId == Guid.Empty) return;
