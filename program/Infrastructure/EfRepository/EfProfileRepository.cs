...
public class EfProfileRepository(TodoContext context) : IProfileRepository
{
		...
		ProfileCriteria profileCriteria)
	{
		...
		{
			query = query.Where(p => profileCriteria.ProfileId.IsSatisfiedBy(Guid.Parse(p.ProfileId)));
		}
		if (profileCriteria.FirstName is not null)
		{
			query = query.Where(p => profileCriteria.FirstName.IsSatisfiedBy(p.FirstName));
		}
		if (profileCriteria.LastName is not null)
		{
			query = query.Where(p => profileCriteria.LastName.IsSatisfiedBy(p.LastName));
		}
		if (profileCriteria.DateOfBirth is not null)
		{
			query = query.Where(p => profileCriteria.DateOfBirth.IsSatisfiedBy(p.DateOfBirth));
		}
		if (profileCriteria.CreatedAt is not null)
		{
			query = query.Where(p => profileCriteria.CreatedAt.IsSatisfiedBy(p.CreatedAt));
		}
